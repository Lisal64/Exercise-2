...
def count_integer(numbers, integer):
    ...
    for n in range(0, i):
        ...
        user_number = [int(input())]  # an empty input to enter the number for the given index
    ...
    for v in range(0, i):  # need to loop through all the numbers to find equal ones
        if numbers[v] == integer:
            count += 1  # adding +1 for every number found - only returns 1, even if there are more
    if count != 0:  # if the for-loop found matches, return the count established in the for-loop
        return count
    else:  # this needs to be outside the loop - otherwise it would always return 42
        not_found = int(42)
        return not_found
...
def compare_lists(list1, list2):
    ...
    return list1  # turning them back into lists
...
# Exercise 5
dictionary = {"Type": "unknown type", "Brand": "unknown brand", "Price": "unknown price"}
